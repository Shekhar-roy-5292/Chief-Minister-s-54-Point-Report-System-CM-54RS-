...
class AdminDashboardController extends Controller
{
    ...
    public function logreport(){
       ...
       $logKCC=DB::table('kishan_credit_card')
             ->select('kishan_credit_card.posted_date','users.name')
             ->join('users','users.id','=','kishan_credit_card.user_code')
             ->orderBy('kishan_credit_card.posted_date','DESC')
             ->first(); 
             
      $logKM=DB::table('kishan_mandi')
             ->select('kishan_mandi.posted_date','users.name')
             ->join('users','users.id','=','kishan_mandi.user_code')
             ->orderBy('kishan_mandi.posted_date','DESC')
             ->first(); 
      $logMgnregs=DB::table('mgnregs')
             ->select('mgnregs.posted_date','users.name')
             ->join('users','users.id','=','mgnregs.user_code')
             ->orderBy('mgnregs.posted_date','DESC')
             ->first(); 
             
      $logAnandadhara=DB::table('anandadhara')
             ->select('anandadhara.posted_date','users.name')
             ->join('users','users.id','=','anandadhara.user_code')
             ->orderBy('anandadhara.posted_date','DESC')
             ->first(); 
               
        return view('admin.log_report',compact('logKCC','logKM','logMgnregs','logAnandadhara'));
    }
}
